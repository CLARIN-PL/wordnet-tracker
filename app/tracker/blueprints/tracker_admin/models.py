...
class AdminQuery(db.Model):
    ...
    type = db.Column('type', db.Enum(AdminQueryTypeEnum),
                     default=AdminQueryTypeEnum.statistic)

    required_words = ('select', 'from')
    banned_words = (
        'update ', 'insert ', 'delete ', 'create ', 'start ', 'begin ',
        'commit ', 'drop ', 'alter ', 'database',
        'show ', 'describe ', 'into ', 'flush ', 'add ', 'modify ', 'load ',
        'truncate ', 'mysql',
    ...
    def validate_query_text(self, key, query):
        ...
        if all([
            ...
            raise ValueError(
                'Query needs to use SELECT and do not modify database in any way.')
    ...
    def execute(query):
        try:
            ...
            headings = aqs.keys()
            if aqs.returns_rows:
                rows = [{key: value for (key, value) in o.items()} for o in
                        aqs]
                connection.close()
            else:
                rows = []
        ...
        if results is None or update:
            ...
            if aqs.returns_rows:
                ...
                if len(aqs):
                    ...
                    cache.set(cache_key, results, timeout=current_app.config[
                        'ADMIN_QUERY_CACHE_TIMEOUT'])
                else:
                    results = [], [], "unknown"
            else:
                results = [], [], "unknown"
